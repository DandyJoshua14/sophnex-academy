<script lang="ts">
	import {getFullCourse, getJavaScriptTranscripts} from '$lib/pocketbase';
...
onMount(async () => {
		const transcripts = await getJavaScriptTranscripts(selectedDay);
		classes = await getFullCourse("javascript")
		console.log(classes);
	...
	let javascriptLessons = $derived(classes);

	

	async function selectDay(day: string) {
		selectedDay = day;
		const transcripts = await getJavaScriptTranscripts(day);
...
<div class="space-y-6">
	<div class="rounded-lg bg-white p-6 shadow-lg">
		...
		<p class="mb-6 text-[#5254A3]">
			This 30-day intensive course will take you from complete beginner to confident JavaScript developer. Each day builds upon the previous lessons, focusing on practical web development skills.
		</p>
