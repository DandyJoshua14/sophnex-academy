<script lang="ts">
	import {getFullCourse, getPythonTranscripts} from '$lib/pocketbase';
...
onMount(async () => {
		const transcripts = await getPythonTranscripts(selectedDay);
		classes = await getFullCourse("python")
		console.log(classes);
	...
	let pythonLessons = $derived(classes);

	

	async function selectDay(day: string) {
		selectedDay = day;
		const transcripts = await getPythonTranscripts(day);
...
This 30-day intensive course will take you from complete beginner to confident Python
			programmer. Each day builds upon the previous lessons, ensuring you have a solid foundation.		</p>
